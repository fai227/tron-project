...
本製品の目的は，micro:bit及びmicro:Maqueen Plus V2 for micro:bit(以降Maqueen)を利用した協調型自動運転の実現である．リアルタイム性を活用した，時空間グリッド予約システムを導入することにより，複数の自動車が相互干渉を行うことなく走行可能になる．<br>本プログラムでは，各車両が走行予定ルートを他の車両と共有し，時空間上のグリッドを予約することで，交通の効率化と安全性の向上を図る．加えて，加えて，リアルタイムOS（RTOS）の特性を活かし，低遅延での処理を実現することで車両の安全性を確保した．サーバーとなるmicro:bit１台と，クライアント車両となるmicro:bitとMaqueen plus V2を３セット用意した．
...
flowchart LR
    ...
    H --> B
```
走行プログラムは「直進」，「L2かR2の反応」，「右左折」，「転回」などの動作を行うことで車両が進行方向を修正しながら自律的に移動する．
走行中，定期的にL2（左のセンサー）やR2（右のセンサー）が障害物などに反応するかどうかを確認し，その結果に基づいて右折または左折が必要か判断する．このフローの中で，経路の何割を超えたかを判断し，設定された条件を超えた場合には，サーバーとの通信プログラムを起動する．<br>
通信プログラムが起動されると，車両はサーバーとの間で経路予約通信を行う．車両はまずサーバーに対して，旅行開始時刻，開始地点，終了地点を送信し，サーバーからは経路指示の配列が返送される．通信パケットは16ビットの旅行開始時刻，旅行開始地点の座標（X,Y），および旅行終了地点の座標で構成される．サーバーが返送するパケットには，経路指示が複数のコマンドとして含まれており，車両はこれに基づいて指定された経路を走行する．
...
flowchart LR
  ...
  L --> N([END待機])
```
サーバープログラムでは，サーバーが車両からの通信リクエストを受信し，処理する流れを示す．サーバーはまず，イベント割り込みが発生した際に，イベントの判定を行う．ここで判定された内容に応じて、送信タスクまたは受信タスクが開始される．
車両との通信は，ID取得通信と経路予約通信の2つが主に行われる．まず，ID取得通信では，車両はサーバーに対して論理アドレスを要求する．この際，車両は初回の通信で論理アドレス0を使用し，サーバーから1〜7の範囲で論理アドレスが返送される．その後，車両はサーバーに対して経路予約通信を行い，旅行開始時刻，開始地点（X, Y），終了地点（X, Y）の情報を送信する．サーバーはこれを受け取り，車両に対して適切な経路指示を含むパケットを返送する．<br>
サーバープログラムでは，RX（受信）とTX（送信）状態を常に監視し，IDLE（待機状態）であれば送信タスクや受信タスクが起動する．また，送信タスクでは，車両がリクエストした経路予約通信に対して，経路指示の配列を返送する．この配列には，移動方向や転回，停止などのコマンドが含まれており，車両が自律的に指定されたルートを進行できる．
...
```
クライアント通信プログラムでは，クライアントがどのようにタスクを起動して通信を行うかを示す．まず，タスクが起動すると，送信タスクを起動する．このタスクは送信割り込みの設定後に通信を実行し，送信が完了すると待機状態に入る．次に，受信割り込みが発生すると，受信タスクが終了し，割り込みが削除され，パケットの読み取りが行われる．このプロセスは繰り返し行われ，クライアントはサーバーと双方向の通信を実行する．
...
上位4ビットについて，この部分は，動作の方向を指示するために使用する．上位4ビットのビットパターンを取り出すためには，orderを4ビット右にシフトする．その結果を対応する方向と比較することで，動作がどの方向を指しているのかを判定できる．<br>
下位4ビットについて，この部分は動作の継続時間を秒単位で表現するために使用する．下位4ビットを取り出すためにビットマスクを使用することにより上位4ビットを無視し，継続時間を抽出することが可能になる．<br>
...
## 実行方法
### 想定環境
本プログラムでは，特定の経路や動作を指定して自動的に移動するシステムを想定した環境で使用されることを前提とする．具体的には，「田」の形をした道路環境を想定する．この環境において，グリッド状に区分されたエリアで，移動方向や経路が明確に決まっており，外周や中央部分は一方通行でのみ走行可能とする．これにより，Maqueenがスムーズに経路を進むことが可能である．<br>
このグリッドは，5×5の構造を持ち、各セルにはIDが割り振られている．セルのIDは，左上から右下に向けて座標平面のように定義されており，例えば左上が(0,0)、右下が(4,4)となる．ただし，特定のエリア（(1,1)，(1,3)，(3,1)，(3,3)）は通行不可能なゾーンとして設定する．以下に道路環境の図を示す．<br>
