...
1. ID取得通信（V: Vehicle, S: Server）
- V → S：空のパケット
- S → V：1バイト目に車両ID，2バイト目に侵入可能時間(秒)
  - micro:bitのnrf5の通信機能における論理アドレスをそのまま車両IDとしているため，今回の実装では7台までしか車両は追加できない．
 
1. 経路予約通信（V: Vehicle, S: Server）
- V → S：1バイト目に旅行開始秒数，2バイト目に旅行開始地点，3バイト目に旅行終了地点
- S → V：1~32バイトに経路指示（必要な分だけ使用，それ以外は0）
...
全てのプログラムは以下URLに記載している．<br>
https://github.com/fai227/tron-project<br>
また，本プログラムはapp\_sampleフォルダに格納されている．
...
5回の実験を行った結果，サーバーの平均CPU使用率は0.20%で，車両の平均CPU使用率は0.89%であった．しかし，車両は割込み発生回数が多いため，短時間のログしか取得することができなかった．
